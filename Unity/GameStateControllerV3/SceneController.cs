...
namespace DouduckGame {
	public sealed class SceneController {
		...
		private void LoadSceneAsync(string sSceneName) {
			Debugger.Log(string.Format("[SceneController] Start Loading Scene<{0:}>", sSceneName));
		...
		private IEnumerator<float> StartLoadScene(string sSceneName) {
			...
			Debugger.Log(string.Format("[SceneController] Loading Scene<{0:}> is done", sSceneName));
		}
	}
}
